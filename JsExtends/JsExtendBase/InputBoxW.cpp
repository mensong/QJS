...
const wchar_t *_lpWndFontName = L"宋体";
...
int WINAPI WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			int len = ::GetWindowTextLength(_hEdit);
			_szBuffer.resize(len + 1);
			::GetWindowText(_hEdit, &_szBuffer[0], len + 1);
			//::GetDlgItemTextW(hWnd, 2000, &_szBuffer[0], len+1);
			::DestroyWindow(hWnd);
		}
			break;
		case IDCANCEL:
